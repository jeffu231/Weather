...
var proxySnippit = require('grunt-connect-proxy/lib/utils').proxyRequest;
...
module.exports = function(grunt) {
					...
					livereload : {
						options : {
							middleware : function(connect) {
								return [ proxySnippit, liveReloadSnippet,
	...
	grunt.registerTask('serve', function(target) {
		if (target === 'dist') {
			return grunt.task.run([ 'jshint', 'build',
					'configureProxies:server', 'connect:production',
					'open:server', 'wait:longtime' ]);
		}

		grunt.task.run([ 'replace:server', 'compass:server',
				'configureProxies:server', 'connect:livereload', 'open:server',
				'focus:dev' ]);
	});
	...
	grunt.registerTask('test', function(isConnected) {
		...
		var testTasks = [ 'clean', 'replace:server', 'compass', 'connect:test',
				'mocha', 'open:test', 'focus:test' ];
